...
def load_data_from_gsheet(url, worksheet_name):
    try:
        ...
        # Tracking Column
        possible = ["Tracking No", "AWB No", "Tracking ID"]
        ...
        # Received Setup
        df['Received'] = df.get('Received', "Not Received")
...
def sync_to_google_sheet(df, url, worksheet_name):
    try:
        ...
        return True, f"✅ Pushed successfully to **{worksheet_name}**"
    except Exception as e:
...
def process_bulk_upload(bulk_file):
    ...
    try:
        ...
        if 'Tracking ID' not in bulk_df.columns:
            st.error("'Tracking ID' column not found in uploaded file")
            return
...
with st.sidebar:
    ...
    sheet_name = st.selectbox("Sheet/Tab", ["Courier Return", "Reverse Pickup"])
